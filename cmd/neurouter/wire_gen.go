...
func wireApp(configConfig config.Config, confServer *conf.Server, data *conf.Data, logger *slog.Logger) (*kratos.App, func(), error) {
	...
	upstreamFactory := anthropic.NewAnthropicChatRepoFactory(loggerProvider, tracerProvider, logger)
	repositoryUpstreamFactory := google.NewGoogleFactory(loggerProvider, tracerProvider, logger)
	upstreamFactory2 := neurouter.NewNeurouterFactory(tracerProvider, logger)
	upstreamFactory3 := openai.NewOpenAIFactory(loggerProvider, tracerProvider, logger)
