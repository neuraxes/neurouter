	...
	Convey("Given an Anthropic MessageNewParams request", t, func() {
		...
		Convey("When converting a complete request with system, messages, tools", func() {
			...
			Convey("Then tools should be converted", func() {
				...
				So(fn.InputSchema.Properties["city"].Type, ShouldEqual, v1.Schema_TYPE_STRING)
				So(fn.InputSchema.Properties["city"].Description, ShouldEqual, "The name of city")
				So(fn.InputSchema.Required, ShouldResemble, []string{"city"})
			})
		})
