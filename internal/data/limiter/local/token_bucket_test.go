...
func TestNewRPMLimiter(t *testing.T) {
	Convey("Test NewRPMLimiter", t, func() {
		...
		Convey("should return nil with zero limit", func() {
			limiter := NewRPMLimiter(0)
			So(limiter, ShouldBeNil)
		})

		Convey("should return nil with negative limit", func() {
...
func TestRequestReservation_Cancel(t *testing.T) {
	...
	Convey("Test requestReservation Cancel idempotency", t, func() {
		Convey("multiple Cancel calls should only refund once", func() {
			limiter := NewRPMLimiter(1)
			...
			// Should have exactly 1 token available (burst=1)
			res2, err := limiter.Reserve()
			So(err, ShouldBeNil)
			So(res2.Delay(), ShouldEqual, 0)

			// Next should be delayed
...
func TestTPMLimiter_Probe(t *testing.T) {
	Convey("Test TPMLimiter Probe", t, func() {
		...
		Convey("probe at exactly burst should return 0", func() {
			limiter := NewTPMLimiter(1000)
			// cost == burst should succeed, not return InfDuration
			delay := limiter.Probe(1000)
			So(delay, ShouldEqual, 0)
		})

		Convey("probe above burst should return InfDuration", func() {
			limiter := NewTPMLimiter(1000)
			delay := limiter.Probe(1001)
			So(delay, ShouldEqual, repository.InfDuration)
		})

		Convey("reserve at exactly burst should succeed", func() {
			limiter := NewTPMLimiter(1000)
...
func TestTokenReservation_Cancel(t *testing.T) {
	...
	Convey("Test tokenReservation Cancel idempotency", t, func() {
		Convey("multiple Cancel calls should only refund once", func() {
			limiter := NewTPMLimiter(1000)
			...
			// Should have full burst available (1000)
			res2, err := limiter.Reserve(1000)
			So(err, ShouldBeNil)
			So(res2.Delay(), ShouldEqual, 0)
...
func TestTokenReservation_CompleteWithActual(t *testing.T) {
	Convey("Test tokenReservation CompleteWithActual", t, func() {
		...
		Convey("Test tokenReservation CompleteWithActual idempotency", func() {
			Convey("multiple CompleteWithActual calls should only adjust once", func() {
				limiter := NewTPMLimiter(1000)

				// Reserve 500 tokens, actually use 300
				res, err := limiter.Reserve(500)
				So(err, ShouldBeNil)

				res.CompleteWithActual(300)
				res.CompleteWithActual(300) // should be no-op
				res.CompleteWithActual(0)   // should be no-op
