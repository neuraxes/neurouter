...
func TestModelGenAITarget(t *testing.T) {
	Convey("Given an OpenAI-compatible model with a configured provider and base URL", t, func() {
		m := &model{
			config: &conf.Model{
				Id:         "router-gpt",
				UpstreamId: "gpt-4.1",
		...
		So(target.RouterModel, ShouldEqual, "router-gpt")
	...
	Convey("Given a chained Neurouter model without an explicit provider", t, func() {
		m := &model{
			...
			upstreamConfig: &conf.UpstreamConfig{
				Name: "remote",
				Config: &conf.UpstreamConfig_Neurouter{
					Neurouter: &conf.NeurouterConfig{
		...
		So(target.Provider, ShouldEqual, genaiconv.ProviderNameAttr(""))
	...
	Convey("Given an upstream without a configurable endpoint", t, func() {
		m := &model{
			config: &conf.Model{Id: "gemini-2.5-pro", Provider: "gcp.gemini"},
			upstreamConfig: &conf.UpstreamConfig{
				Name:   "google-primary",
				Config: &conf.UpstreamConfig_Google{Google: &conf.GoogleConfig{}},
			},
		}

		target := m.GenAITarget()

		So(target.Provider, ShouldEqual, genaiconv.ProviderNameGCPGemini)
		So(target.ServerAddress, ShouldBeEmpty)
		So(target.ServerPort, ShouldEqual, 0)
	})
}
