...
type Content struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The upstream item this content was derived from. Several contents share one
	// value when the upstream packs them into a single item. Neurouter replays it
	// to upstreams that require items to be referenced by id, and leaves it
	// empty for upstreams that expose no item identity.
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
